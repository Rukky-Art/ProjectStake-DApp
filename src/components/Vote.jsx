  ...
  return (
    <div>
      <div className="row" style={{marginBottom:8}}>
        <button onClick={()=>castVote(1)} disabled={loading}>Vote Proposal 1</button>
        <button onClick={()=>castVote(2)} disabled={loading}>Vote Proposal 2</button>
      </div>

      {status && <div className="status">
        <strong>{status.type === 'error' ? 'Error' : status.type === 'success' ? 'Success' : 'Info'}:</strong>
        <span style={{marginLeft:8}}>{status.msg}</span>
      </div>}

      {choice && <div style={{marginTop:8}}>You voted for proposal <strong>{choice}</strong></div>}
    </div>
  )
}
